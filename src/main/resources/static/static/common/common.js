    ...
    ajaxExtend : function(){
        ...
        if(sessionStorage.getItem('sysApiEncrypt') === "Y" && !jQueryExtend.ajaxExtendFlag){
            ...
            $.ajax = function (opt) {
                ...
                //BuJiaMi请求不加密
                //请求地址最后是BuJiaMi的使用原ajax
                let zuiHouUrl = opt.url;
                let index = zuiHouUrl.lastIndexOf("\/");
                zuiHouUrl  = zuiHouUrl.substring(index + 1, zuiHouUrl.length);
                let buJiaMi = zuiHouUrl.indexOf("BuJiaMi")>-1;
                //返回原ajax
                if(buJiaMi){
                    jQueryExtend.ajaxExtendFlag=false;
                    return _ajax(opt);
                }
                //console.log(indexOf)
