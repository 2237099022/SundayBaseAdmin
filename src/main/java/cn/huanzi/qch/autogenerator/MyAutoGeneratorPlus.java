package cn.huanzi.qch.autogenerator;
...
public class MyAutoGeneratorPlus {
    ...
    //设置路径
    String codePath = System.getProperty("user.dir") + "\\code";
    ...
    private static final String PASSWORD = "yyfj8229_";
     ...
     */
    //模板文件位置
    private String tlfPath = System.getProperty("user.dir") + "\\src\\main\\resources\\tlf\\";
    //后台代码位置
    private String filePackage = "cn\\huanzi\\qch\\mycode\\"+""+"\\";
    //前端文件位置
    //html
    private String htmlPath = codePath + "\\main\\resources\\view\\myview\\"+""+"\\";
    
    //js css
    private String jscssPath = codePath + "\\main\\resources\\static\\myjscss\\"+""+"\\";
    

    /**
     * 构造参数，设置表名
     */
    private MyAutoGeneratorPlus(String tableName) {
        //设置表名
        ...
        filePath = codePath + "\\main\\java\\" + filePackage;
    }
    
    /**
     * 构造参数，设置表名
     */
    private MyAutoGeneratorPlus(String tableName,String moKuaiMingParm) {

        moKuaiMing=moKuaiMingParm;

        //后台代码位置
        filePackage = "cn\\huanzi\\qch\\mycode\\"+moKuaiMing+"\\";
        //前端文件位置
        //html
        htmlPath = codePath + "\\main\\resources\\view\\myview\\"+moKuaiMing+"\\";
        //js css
        jscssPath = codePath + "\\main\\resources\\static\\myjscss\\"+moKuaiMing+"\\";
    ...
    private void writer(String templatePath, String outputFile,List<TableInfo> tableInfos,Map<String,String> customParameter){
        ...
        try(FileReader fileReader = new FileReader(templatePath);
            ...
            for (Object o : lines) {
                ...
                if(line.contains("${moKuaiMing}")){
                    line = line.replaceAll("\\$\\{moKuaiMing}", moKuaiMing);
                }
    ...
    private String create() {
        ...
        customParameter.put("author","作者：sunday'");
        ...
        writer(tlfPath+"entityvo.tlf",
                filePath + "pojo\\" + captureName + "Vo.java",
                tableInfo,customParameter);
        ...
        //html
        writer(tlfPath+"html.tlf",
                htmlPath + captureName.toLowerCase()+"\\" + captureName.toLowerCase() + ".html",
                tableInfo,customParameter);
        //js
        writer(tlfPath+"js.tlf",
                jscssPath + captureName.toLowerCase()+"\\" + captureName.toLowerCase() + ".js",
                tableInfo,customParameter);
        //css
        writer(tlfPath+"css.tlf",
                jscssPath + captureName.toLowerCase()+"\\css\\" + captureName.toLowerCase() + ".css",
                tableInfo,customParameter);
        ...
        String sql="INSERT INTO sys_menu(menu_id,menu_name,menu_path,menu_parent_id,sort_weight,create_time,update_time) VALUES ('巴巴','baba');";
        return tableName + " -- 后台代码生成完毕！"+"\n-- 语句sql语句\n"+"INSERT INTO sys_menu(menu_id,menu_name,menu_path,menu_parent_id,sort_weight,create_time,update_time) VALUES ('"+UUIDUtil.getUuid()+"','"+tableComment+"','/mycode/"+captureName.toLowerCase()+"/index','b0bf3b8aa6624d38b54dc0bddc3abd14','50','"+date+"','"+date+"');";
    }

    public static void main(String[] args) {
//        String[] tables = {"sys_user","sys_menu","sys_authority","sys_user_menu","sys_user_authority","sys_shortcut_menu","sys_setting"};
        //对应数据库表
        String[] tables = {"yyfj_xuhui_allhy","yyfj_xuhui_caozuojilu","yyfj_xuhui_data","yyfj_xuhui_table"};
        //模块文件夹名
        String moKuaiMing="xuhui";
        for (String table : tables) {
            String msg = new MyAutoGeneratorPlus(table,moKuaiMing).create();
